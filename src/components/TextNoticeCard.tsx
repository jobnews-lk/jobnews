      ...
      <div className="p-4 md:p-5 flex-1 flex flex-col">
        ...
        <div className="mt-auto pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-1.5 flex-wrap">
          <div className="flex items-center gap-1.5 flex-wrap">
            <Link to={`/jobs/${job.id}`} className="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg transition-colors shadow-sm">
              Notice <ChevronRight className="w-3.5 h-3.5" />
            </Link>
            {hasApplyUrl && (
              <a href={job.apply_url!} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 px-2.5 py-1.5 border border-blue-200 dark:border-blue-800/50 text-blue-600 dark:text-blue-400 text-xs font-medium rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors">
                <ExternalLink className="w-3 h-3" /> Apply
              </a>
            )}
            {job.apply_method === 'email' && job.apply_email && (
              <a href={`mailto:${job.apply_email}`} className="inline-flex items-center gap-1 px-2.5 py-1.5 border border-blue-200 dark:border-blue-800/50 text-blue-600 dark:text-blue-400 text-xs font-medium rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors">
                <Mail className="w-3 h-3" /> Email
              </a>
            )}
            {job.apply_method === 'phone' && job.apply_phone && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-medium rounded-lg">
                <Phone className="w-3 h-3" /> {job.apply_phone}
              </span>
            )}
          ...
          <div className="flex items-center gap-1.5 ml-auto">
            ...
            <Link to={`/jobs/${job.id}`} className="inline-flex items-center gap-1 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/20 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/50 text-xs font-semibold rounded-lg transition-colors shadow-sm">
              Details <ChevronRight className="w-3.5 h-3.5" />
            </Link>
            {/* Share Button at the far right end */}
            <button
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
