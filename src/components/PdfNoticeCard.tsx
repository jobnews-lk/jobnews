...
import { MapPin, Calendar, Clock, Globe, Landmark, FileText, X, Eye, Download, ExternalLink, ChevronRight, Mail, Phone, MapPin as MapPinIcon, Share2, Briefcase, Building2 } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden hover:shadow-md transition-shadow">
      ...
      <div className="p-4 md:p-5 flex-1 flex flex-col">
        ...
        {job.company && (
          <div className="flex items-center gap-1.5 text-xs md:text-sm text-slate-500 dark:text-slate-400 mb-2">
        ...
        {/* PDF Preview Card */}
        <div className="flex items-center gap-3 bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-100 dark:border-red-900/30 p-3 mb-3">
        ...
        {job.description && (
          <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed line-clamp-2 mb-3 bg-slate-50 dark:bg-slate-800/40 p-2 rounded-lg border border-slate-100 dark:border-slate-800">
            {job.description}
          </p>
        )}
        ...
        <div className="mt-auto pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2">
          {pdfUrl && (
            <button onClick={() => setViewerOpen(true)} className="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg transition-colors shadow-sm shrink-0">
